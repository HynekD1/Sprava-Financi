...
## 1. Zadání projektu
Cílem projektu je vytvořit konzolovou aplikaci v jazyce C#, která uživateli umožní evidovat finanční příjmy a výdaje, zaznamenávat jejich popis a kategorie a sledovat celkový stav rozpočtu v čase s trvalým ukládáním dat.
...
* `public int SmazJednuTransakciPodleData(int den, int mesic, int rok)` - Vyhledá transakce odpovídající zadanému datu, nabídne uživateli jejich očíslovaný přehled a vymaže ze seznamu pouze jednu vybranou položku.
...
## 5. Popis ovládání
Program se kompletně ovládá textově přes konzolu zadáváním číselných voleb v hlavním menu. Po každé akci je uživatel vyzván ke stisku klávesy Enter pro návrat do menu:
1. **Přidat transakci:** Výzva k zadání parametrů transakce. Zadávání částky (nesmí být záporná), dne (1-31), měsíce (1-12) a roku (0-2026) je ošetřeno pomocí cyklů `do-while`, které uživatele nepustí dál při zadání neplatné hodnoty. Následuje zadání textového popisu, kategorie a typu transakce (1 = příjem / 0 = výdej).
...
5. **Smazat transakce podle data:** Uživatel zadá konkrétní den, měsíc a rok. Program vyhledá transakce pro dané datum, nabídne uživateli jejich očíslovaný seznam a umožní smazat jednu konkrétní zvolenou transakci.
6. **Uložit a odejít:** Zapíše veškeré změny z operační paměti do souboru a bezpečně ukončí běžící aplikaci.
